    ...
    <div class="w-full px-4 sm:px-6 md:px-16 py-2 sm:py-3">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2">
                ...
                <span class="hidden sm:block text-gray-900 font-bold text-lg lg:text-md tracking-tight ml-2">
            ...
            <div class="flex items-center space-x-4">
                ...
                @else
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" style="display:none;" class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-100 max-h-96 overflow-y-auto z-50">
                            ...
                            @forelse($notifications as $notif)
                                <a href="{{ route('notifications.read', $notif->id_notification) }}" class="block p-4 hover:bg-gray-50 transition {{ !$notif->is_read ? 'bg-indigo-50' : '' }}">
                    ...
                    {{-- ─── Avatar + Dropdown (tampil di SEMUA halaman) ─── --}}
                    @php $avatarUrl = $authUser->avatar_url; @endphp
                    <div class="relative">
                        @if($isDashboard)
                            <button onclick="toggleDropdown()"
                                class="flex items-center gap-2 focus:outline-none hover:bg-gray-50 rounded-lg px-2 py-1.5 transition">
                                <div
                                    class="w-8 h-8 rounded-full overflow-hidden ring-2 ring-gray-200 flex items-center justify-center text-white font-semibold bg-gradient-to-br from-indigo-500 to-purple-600">
                                    @if($avatarUrl)
                                        ...
                                        <div class="w-full h-full flex items-center justify-center text-white font-semibold bg-gradient-to-br from-indigo-500 to-purple-600" style="display:none;">{{ strtoupper(substr($authUser->nama, 0, 1)) }}</div>
                                    @else
                                        {{ strtoupper(substr($authUser->nama, 0, 1)) }}
                                    @endif
                                </div>
                                <span class="hidden sm:inline text-gray-700 font-medium">{{ $authUser->nama }}</span>
                            ...
                            <div id="userDropdown"
                                ...
                                <button onclick="openLogoutModal()" class="w-full flex items-center gap-3 text-left px-4 py-3 text-red-600 hover:bg-red-50 transition">
                        ...
                        @else
                            <div class="flex items-center gap-2 px-2 py-1.5">
                                <div class="w-8 h-8 rounded-full overflow-hidden ring-2 ring-gray-200 flex items-center justify-center text-white font-semibold bg-gradient-to-br from-indigo-500 to-purple-600">
                                    @if($avatarUrl)
                                        <img src="{{ $avatarUrl }}" alt="Avatar" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">
                                        <div class="w-full h-full flex items-center justify-center text-white font-semibold bg-gradient-to-br from-indigo-500 to-purple-600" style="display:none;">{{ strtoupper(substr($authUser->nama, 0, 1)) }}</div>
                                    @else
                    ...
                    {{-- Action Button --}}
                    @php $role = $authUser->role; @endphp
                    @if($accountPage)
                        <button onclick="window.history.back()" class="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition shadow-md">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @elseif($isDashboard)
                        @if($role === 'siswa')
                            <button onclick="openJoinClassModal()" class="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition shadow-md">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @elseif($role === 'guru')
                            <button onclick="openCreateClassModal()" class="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition shadow-md">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6" />
                                </svg>
                        ...
                        @elseif($role === 'admin')
                            <button onclick="openManageClassModal()" class="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition shadow-md">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6" />
                                </svg>
