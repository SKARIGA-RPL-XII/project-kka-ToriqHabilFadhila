...
<nav class="w-full bg-white border-b border-gray-100 sticky top-0 z-40 transition-all duration-300"
        ...
        <div class="flex justify-between items-center gap-4">
            <a href="{{ auth()->check() ? route('dashboard') : '/' }}" class="flex items-center gap-2.5 flex-shrink-0"
                ...
                <span class="hidden sm:block font-bold text-gray-900 text-base tracking-tight leading-tight">
                    Learning<br>
                    <span
                        style="background:var(--brand-gradient);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:0.78rem;font-weight:700;letter-spacing:0.01em;">Management
                        System</span>
                </span>
            </a>
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    ...
                    {{-- ✅ Notifikasi & Profile Dropdown: HANYA tampil di Dashboard --}}
                    @if ($isDashboard)
                        @php
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                class="notif-panel" style="display:none;">
                                <div class="notif-header">
                                    <h3>Notifikasi</h3>
                                    @if ($unreadCount > 0)
                                        <span
                                            style="font-size:0.72rem;font-weight:600;padding:2px 8px;border-radius:99px;background:linear-gradient(135deg,rgba(124,58,237,0.1),rgba(219,39,119,0.1));color:#7c3aed;">
                                            {{ $unreadCount }} baru
                                        </span>
                                    @endif
                                </div>
                                ...
                                @forelse($notifications as $notif)
                                    <a href="{{ route('notifications.read', $notif->id_notification) }}"
                                        ...
                                        <div
                                            class="notif-dot {{ $notif->type === 'deadline' ? 'bg-red-100' : ($notif->type === 'grade' ? 'bg-green-100' : 'bg-purple-100') }}">
                                        ...
                                        <div style="flex:1;min-width:0;">
                                            <p style="font-size:0.82rem;font-weight:600;color:#111827;line-height:1.3;">
                                                {{ $notif->title }}</p>
                                            <p style="font-size:0.75rem;color:#6b7280;margin-top:2px;line-height:1.4;">
                                                {{ $notif->message }}</p>
                                            <p style="font-size:0.7rem;color:#9ca3af;margin-top:3px;">
                                                {{ $notif->created_at->diffForHumans() }}</p>
                                        </div>
                                        @if (!$notif->is_read)
                                            <div
                                                style="width:7px;height:7px;border-radius:50%;background:var(--brand-from);flex-shrink:0;margin-top:4px;">
                                            </div>
                                        @endif
                                    </a>
                                @empty
                                    <div style="padding:2.5rem 1rem;text-align:center;color:#9ca3af;">
                                        <svg class="w-10 h-10 mx-auto mb-2" style="color:#d1d5db;" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="relative" style="position:relative;">
                            <button onclick="toggleDropdown()"
                                class="flex items-center gap-2 hover:bg-gray-50 rounded-xl px-2 py-1.5 transition border border-transparent hover:border-gray-200">
                                <div class="avatar-ring">
                                    <div class="avatar-inner">
                                        @if ($avatarUrl)
                                            ...
                                            <span
                                                style="display:none;">{{ strtoupper(substr($authUser->nama, 0, 1)) }}</span>
                                        @else
                                ...
                                <span
                                    class="hidden sm:inline text-gray-700 font-semibold text-sm">{{ $authUser->nama }}</span>
                                <svg class="hidden sm:block w-3.5 h-3.5 text-gray-400" fill="none"
                        ...
                        @endif
                    @else
                        {{-- ✅ Non-Dashboard: Tampilkan nama user + tombol Logout --}}
                        <div class="flex items-center gap-2">
                            {{-- Nama User --}}
                            ...
                            {{-- Tombol Kembali --}}
                            <a href="{{ $backUrl }}" class="nav-btn-ghost">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
