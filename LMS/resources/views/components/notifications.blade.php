...
<div id="loading-overlay" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[10000] hidden items-center justify-center">
    <div class="bg-white rounded-2xl p-8 max-w-sm mx-4 text-center shadow-2xl" id="loading-content">
        <div class="w-20 h-20 mx-auto mb-4 relative">
            ...
            <div class="absolute inset-0 border-4 border-transparent border-t-purple-600 rounded-full animate-spin">
            </div>
            <div class="absolute inset-2 border-4 border-transparent border-t-pink-400 rounded-full animate-spin"
        ...
        <div class="mt-4 flex justify-center gap-1">
            <div class="w-2 h-2 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full animate-bounce"
...
<div id="notification-container" class="fixed top-20 right-4 z-[9999] space-y-3 max-w-sm w-full px-4 sm:px-0">
    @if ($errors->any())
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 6000)"
            ...
            x-transition:leave-end="translate-x-full opacity-0 scale-95" class="notification-item notification-error">
    ...
    @if (session('success'))
        <div x-data="{ show: true, progress: 0 }" x-show="show" x-init="setTimeout(() => show = false, 3000);
        let start = Date.now();
        ...
        @endif"
            ...
            x-transition:leave="transform ease-in duration-300"
            x-transition:leave-start="translate-x-0 opacity-100 scale-100"
            x-transition:leave-end="translate-x-full opacity-0 scale-95" class="notification-item notification-success">
    ...
    @if (session('warning'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
            ...
            x-transition:leave="transform ease-in duration-300"
            x-transition:leave-start="translate-x-0 opacity-100 scale-100"
            x-transition:leave-end="translate-x-full opacity-0 scale-95" class="notification-item notification-warning">
            <div class="notification-icon icon-bg">
                <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if (session('info'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
            ...
            x-transition:leave="transform ease-in duration-300"
            x-transition:leave-start="translate-x-0 opacity-100 scale-100"
            x-transition:leave-end="translate-x-full opacity-0 scale-95" class="notification-item notification-info">
            <div class="notification-icon icon-bg">
                <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
