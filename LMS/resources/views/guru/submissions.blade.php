...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    ...
    <div class="w-full px-4 sm:px-6 md:px-16 py-12">
        <div
            class="bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 rounded-3xl shadow-2xl p-6 sm:p-8 mb-8 text-white overflow-hidden relative animate-fade-in-up">
            ...
            <div class="relative z-10">
                ...
                <p class="text-base sm:text-lg text-purple-100 mb-4">{{ $assignment->judul }} -
                    {{ $assignment->class->nama_kelas }}</p>
                <div class="flex flex-wrap gap-2 sm:gap-3">
                    <div class="bg-white/20 backdrop-blur-sm px-3 sm:px-4 py-2 rounded-xl">
                        <span class="text-sm sm:text-base font-semibold">Total Siswa:
                            {{ $assignment->class->enrollments->count() }}</span>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm px-3 sm:px-4 py-2 rounded-xl">
                        <span class="text-sm sm:text-base font-semibold">Sudah Submit:
                            {{ $assignment->submissions->count() }}</span>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm px-3 sm:px-4 py-2 rounded-xl">
                        <span class="text-sm sm:text-base font-semibold">Belum Submit:
                            {{ $assignment->class->enrollments->count() - $assignment->submissions->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="p-4 sm:p-6 hover:bg-gray-50 transition-all duration-200">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                            <div class="flex items-center gap-3 sm:gap-4">
                                <div
                                    class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 flex items-center justify-center text-white font-bold text-base sm:text-lg flex-shrink-0 shadow-md shadow-purple-500/30">
                                    {{ strtoupper(substr($submission->user->nama, 0, 1)) }}
                                </div>
                                <div class="min-w-0">
                                    <h3 class="font-bold text-gray-900 text-sm sm:text-base truncate">
                                        {{ $submission->user->nama }}</h3>
                                    <p class="text-xs sm:text-sm text-gray-500 truncate">{{ $submission->user->email }}
                                    </p>
                                </div>
                            </div>
                            <div class="text-left sm:text-right flex-shrink-0">
                                @if ($submission->status === 'graded')
                                    <span
                                        class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-100 text-green-700 text-xs font-bold rounded-full">
                                    ...
                                    <p class="text-sm font-bold text-green-600 mt-1">Nilai:
                                        {{ $submission->score }}/{{ $assignment->max_score }}</p>
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 px-3 py-1.5 bg-amber-100 text-amber-700 text-xs font-bold rounded-full">
                                        <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <p class="text-xs sm:text-sm text-gray-600 flex items-center gap-1">
                                ...
                                <span class="truncate">Dikumpulkan:
                                    {{ $submission->submitted_at->format('d M Y, H:i') }}</span>
                            </p>
                            <button onclick="toggleAnswer({{ $submission->id_submission }})"
                                class="inline-flex items-center gap-1 text-purple-600 hover:text-purple-700 font-semibold text-sm hover:bg-purple-50 px-3 py-1.5 rounded-lg transition-all">
                                <span>Lihat Detail</span>
                                <svg class="w-4 h-4 transition-transform duration-200"
                                    id="arrow-{{ $submission->id_submission }}" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7" />
                                </svg>
                        ...
                        <div id="answer-{{ $submission->id_submission }}"
                            class="hidden mt-4 p-3 sm:p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-gray-200 answer-detail">
                                                ...
                                                @if ($assignment->tipe === 'pilihan_ganda')
                                                    @php
                                                        $selectedOption = $question->options
                                                            ->where('id_option', $answer)
                                                            ->first();
                                                    @endphp
                                                    <p class="text-gray-700">
                                                        Jawaban: <span
                                                            class="font-semibold {{ $selectedOption && $selectedOption->is_correct ? 'text-green-600' : 'text-red-600' }}">
                                                ...
                                                @else
                                                    ...
                                                    @if ($question->kunci_jawaban)
                                                        <div class="mt-2 p-2 bg-blue-50 rounded border border-blue-200">
                                                            <p
                                                                class="text-xs font-semibold text-blue-700 mb-1 flex items-center gap-1">
                                    ...
                                    <div class="flex items-center justify-between bg-white p-3 rounded-lg">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">Dikumpulkan:
                                                    {{ $submission->submitted_at->format('d M Y, H:i') }}</p>
                                            </div>
                                        </div>
                                        <a href="{{ Storage::url($submission->file_path) }}" target="_blank"
                                            class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-semibold rounded-lg transition-all shadow-md hover:shadow-lg">
                            ...
                            @if ($submission->status !== 'graded')
                                @if (
                                    ($assignment->tipe === 'essay' || $assignment->tipe === 'praktik') &&
                                        $assignment->questions->where('kunci_jawaban', '!=', null)->count() > 0)
                                    <div class="mt-4 mb-3 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                                        <p class="text-xs text-blue-700 mb-2">
                                            ...
                                            <strong>AI Grading:</strong> Klik tombol di bawah untuk koreksi otomatis
                                            dengan AI.
                                        </p>
                                        <button onclick="autoGrade({{ $submission->id_submission }})"
                                            id="ai-btn-{{ $submission->id_submission }}"
                                ...
                                <div id="ai-result-{{ $submission->id_submission }}"
                                    class="hidden mt-3 p-3 rounded-xl">
                                ...
                                <p class="text-xs text-gray-500 mt-2 flex items-start gap-1">
                                    ...
                                    <span><strong>Penilaian Manual:</strong> Atau isi nilai manual di kolom di
                                        bawah.</span>
                                </p>

                                <form method="POST"
                                    action="{{ route('guru.submissions.grade', $submission->id_submission) }}"
                                    id="grade-form-{{ $submission->id_submission }}"
                                    class="flex flex-col sm:flex-row gap-3 mt-3">
                                    @csrf
                                    @method('PUT')
                                    <input type="number" name="score" id="score-{{ $submission->id_submission }}"
                                        min="0" max="{{ $assignment->max_score }}"
                                        placeholder="Nilai (0-{{ $assignment->max_score }})"
                                        class="flex-1 rounded-xl border-2 border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all"
                                        required>
                                    <button type="submit"
                                        class="px-4 sm:px-6 py-2.5 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 text-white text-sm rounded-xl hover:from-slate-800 hover:via-blue-800 hover:to-indigo-700 hover:scale-105 transition-all duration-300 font-semibold shadow-lg shadow-indigo-500/50 hover:shadow-indigo-600/70">
                                        Beri Nilai
                                    </button>
                                </form>
                            @endif
                ...
                if (data.success) {
                    ...
                    if (data.feedback.includes('Error:')) {
                        ...
                        feedbackP.innerHTML =
                            ...
                            '<span class="text-xs text-gray-600 mt-1 block">Detail: ' + data.feedback.replace('Error: ',
                                '') + '</span>';
                    } else {
                    ...
                    btn.innerHTML =
                        '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg> Selesai! Nilai: ' +
                        data.score;
                } else {
                    const resultDiv = document.getElementById(`ai-result-${submissionId}`);
                    const feedbackP = document.getElementById(`ai-feedback-${submissionId}`);
                    ...
                    feedbackP.innerHTML =
                        ...
                        '<span class="text-xs text-gray-600 mt-1 block">Detail: ' + (data.message ||
                        'Gagal mengoreksi') + '</span>';
                    resultDiv.classList.remove('hidden');
                    btn.disabled = false;
