...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    ...
    <div class="w-full px-4 sm:px-6 md:px-12 py-8">
        <div
            class="bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 rounded-2xl shadow-xl p-6 mb-6 text-white animate-fade-in-up overflow-hidden relative">
            ...
            <div class="relative z-10">
                <div class="flex items-start gap-4 mb-6">
                    <div class="flex-1">
                        ...
                        <div class="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl">
                            ...
                            <span class="text-sm">
                                Token: <span
                                    class="font-mono font-bold">{{ $kelas->activeToken->token_code ?? '-' }}</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    <div
                        class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-white/30 transition-all">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path
                                d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
                        </svg>
                        <span class="font-semibold">{{ $kelas->enrollments->count() }}/{{ $kelas->max_students }}
                            Siswa</span>
                    </div>
                    <div
                        class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-white/30 transition-all">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
                <button onclick="openUploadMateriModal()"
                    ...
                    <div
                        class="w-12 sm:w-14 h-12 sm:h-14 rounded-xl bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-all duration-300 shadow-lg shadow-indigo-500/25 group-hover:shadow-indigo-600/40">
            ...
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden animate-fade-in-up">
                ...
                <div class="p-4 max-h-[600px] overflow-y-auto">
                    @forelse($kelas->enrollments as $index => $enrollment)
                        <div class="student-card flex items-center gap-3 p-3 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent mb-2"
                            style="animation-delay: {{ $index * 0.05 }}s">
                            <div
                                class="w-10 h-10 rounded-full bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 text-white flex items-center justify-center text-sm font-bold shadow-md shadow-purple-500/30 flex-shrink-0">
                ...
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden animate-fade-in-up">
                    <div class="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-5 border-b border-purple-100">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div
                                    class="p-2.5 rounded-xl bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800">
                                    <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                            ...
                            <span
                                class="text-xs font-medium text-gray-500 bg-white px-3 py-1.5 rounded-full shadow-sm">
                    ...
                    <div x-data="{ showAll: false }" class="p-6 space-y-4 max-h-[600px] overflow-y-auto">
                        @forelse($kelas->assignments as $index => $assignment)
                            <div x-show="showAll || {{ $index }} < 3" x-cloak
                                class="assignment-card bg-gradient-to-r from-white to-gray-50 border border-gray-200 rounded-2xl p-6 hover:border-purple-300"
                                        ...
                                        <div class="flex flex-wrap items-center gap-3 text-sm ml-11">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-gray-500">
                                                    Deadline: <span
                                                        class="font-semibold text-gray-700">{{ is_string($assignment->deadline) ? $assignment->deadline : $assignment->deadline->format('d M Y, H:i') }}</span>
                                                </span>
                                                @php
                                                    ...
                                                    $daysLeft = now()
                                                        ->startOfDay()
                                                        ->diffInDays($deadline->startOfDay(), false);
                                                @endphp
                                                @if ($daysLeft >= 0)
                                                    @php
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="ml-2 px-2 py-0.5 text-xs font-medium rounded-full {{ $badgeClass }}">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-gray-500">
                                                    <span
                                                        class="font-semibold text-purple-600">{{ $assignment->questions->count() }}</span>
                                                    Soal
                                                </span>
                                            </div>
                                            @php
                                                $submissionCount = $assignment
                                                    ->submissions()
                                                    ->where('status', '!=', 'draft')
                                                    ->count();
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-gray-500">
                                                    <span
                                                        class="font-semibold text-green-600">{{ $submissionCount }}/{{ $totalStudents }}</span>
                                                    siswa mengumpulkan
                                                </span>
                                            </div>
                                        </div>
                        ...
                        @empty
                            <div class="text-center py-16">
                                ...
                                <button onclick="openCreateTugasModal()"
                                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 text-white text-sm font-semibold rounded-xl hover:from-slate-800 hover:via-blue-800 hover:to-indigo-700 hover:scale-105 transition-all duration-300 shadow-lg shadow-indigo-500/50 hover:shadow-indigo-600/70">
                        ...
                        @if ($kelas->assignments->count() > 3)
                            <div class="text-center pt-4">
                                <button @click="showAll = !showAll"
                                    class="text-purple-600 hover:text-purple-700 font-semibold flex items-center gap-1 mx-auto">
                                    <span x-text="showAll ? 'Lihat Sedikit' : 'Lihat Semua'"></span>
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path x-show="!showAll" stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden animate-fade-in-up">
                    ...
                    <div x-data="{ showAll: false }" class="p-6 space-y-3 max-h-[400px] overflow-y-auto">
                        @forelse($kelas->materials as $index => $material)
                            <div x-show="showAll || {{ $index }} < 3" x-cloak
                                class="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition">
                                ...
                                @if ($material->file_path)
                                    <a href="{{ Storage::url($material->file_path) }}" target="_blank"
                                        class="px-4 py-2 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 text-white text-sm font-semibold rounded-xl hover:from-slate-800 hover:via-blue-800 hover:to-indigo-700 hover:scale-105 transition-all duration-300 shadow-lg shadow-indigo-500/50 hover:shadow-indigo-600/70">
                                        Download
                                    </a>
                                @endif
                            </div>
                        ...
                        @if ($kelas->materials->count() > 3)
                            <div class="text-center pt-4">
                                <button @click="showAll = !showAll"
                                    class="text-blue-600 hover:text-blue-700 font-semibold flex items-center gap-1 mx-auto">
                                    <span x-text="showAll ? 'Lihat Sedikit' : 'Lihat Semua'"></span>
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path x-show="!showAll" stroke-linecap="round" stroke-linejoin="round"
                                            stroke-width="2" d="M19 9l-7 7-7-7" />
