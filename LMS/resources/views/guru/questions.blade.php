...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    ...
    <div class="w-full px-4 sm:px-6 md:px-16 py-12">
        <div
            class="bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 rounded-3xl shadow-2xl p-8 mb-8 text-white animate-fade-in-up overflow-hidden relative">
                ...
                <div class="flex items-start gap-4">
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3 text-sm">
                            ...
                            <span
                                class="bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-full font-medium flex items-center gap-2">
        ...
        @if (session('success'))
            <div
                class="animate-fade-in-up bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-500 text-green-800 px-6 py-4 rounded-xl mb-8 shadow-md flex items-start gap-3">
        ...
        <div class="flex gap-4 mb-8">
            <button onclick="openAIGenerateModal()"
                class="flex-1 bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-4 px-6 rounded-2xl font-bold text-lg hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 hover:shadow-xl flex items-center justify-center gap-3">
        ...
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden mb-8 animate-fade-in-up">
            ...
            <form method="POST" action="{{ route('guru.questions.store', $assignment->id_assignment) }}"
                id="questionForm" class="p-8">
                @csrf
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="soal" rows="3" required
                            class="w-full px-3 py-2.5 text-sm border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all resize-none"
                        ...
                        @if ($assignment->tipe === 'essay')
                            <p class="mt-2 text-xs text-gray-500 flex items-start gap-1">
                                ...
                                <span>Tips: Buat pertanyaan yang mendorong siswa untuk menjelaskan, menganalisis, atau
                                    memberikan pendapat.</span>
                            </p>
                        @elseif($assignment->tipe === 'praktik')
                            <p class="mt-2 text-xs text-gray-500 flex items-start gap-1">
                                <svg class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <span>Tips: Berikan instruksi yang jelas tentang apa yang harus dikerjakan dan format
                                    file yang harus diupload.</span>
                            </p>
                        @endif
                    </div>
                    ...
                    @if ($assignment->tipe === 'essay' || $assignment->tipe === 'praktik')
                        <div>
                            ...
                            <textarea name="kunci_jawaban" rows="2"
                                class="w-full px-3 py-2.5 text-sm border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all resize-none"
                            ...
                            <p class="mt-2 text-xs text-purple-600 flex items-start gap-1">
                                ...
                                <span>Kunci jawaban ini akan digunakan oleh AI untuk mengoreksi jawaban siswa secara
                                    otomatis</span>
                            </p>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <input type="number" name="poin" value="10" min="1" required
                            class="w-full px-3 py-2.5 text-sm border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                    </div>
                    ...
                    @if ($assignment->tipe === 'pilihan_ganda')
                        <div id="pilihanContainer">
                            ...
                            <div class="space-y-3 bg-gray-50 p-4 rounded-xl" id="optionsList">
                                <div
                                    class="option-item flex gap-3 items-center bg-white p-3 rounded-lg border-2 border-gray-200 hover:border-purple-300 transition-all">
                                    <input type="radio" name="jawaban_benar" value="0" required
                            ...
                            <button type="button" onclick="addOption()"
                                class="mt-3 inline-flex items-center gap-2 text-sm text-purple-600 hover:text-purple-700 font-semibold hover:bg-purple-50 px-4 py-2 rounded-lg transition-all">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        @if ($assignment->questions->count() > 0)
                            <input type="checkbox" id="selectAll" onclick="toggleSelectAll()"
                                class="w-5 h-5 text-purple-600 focus:ring-purple-500 rounded cursor-pointer">
                        @endif
                        <div class="p-3 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 rounded-xl">
                    ...
                    <div class="flex items-center gap-3">
                        <button id="deleteSelectedBtn" onclick="deleteSelected()"
                            class="hidden items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-semibold">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="p-6 space-y-4">
                @if ($assignment->questions->count() > 0)
                    <div class="mb-6 flex justify-end">
                        <form method="POST"
                            action="{{ route('guru.assignments.publish', $assignment->id_assignment) }}">
                            @csrf
                            <button type="submit"
                                class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition font-semibold shadow-lg">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M5 13l4 4L19 7" />
                                </svg>
                ...
                <div id="questionsContainer">
                    @forelse($assignment->questions as $question)
                        <div class="question-card border-2 border-gray-200 rounded-2xl p-6 hover:border-purple-300 {{ $loop->index >= 3 ? 'hidden-question' : '' }}"
                            x-data="{ edit: false }" style="{{ $loop->index >= 3 ? 'display: none;' : '' }}">
                            <div x-show="!edit">
                                <div class="flex justify-between items-start gap-4">
                                    <input type="checkbox"
                                        class="question-checkbox mt-2 w-5 h-5 text-purple-600 focus:ring-purple-500 rounded cursor-pointer"
                                        data-id="{{ $question->id_question }}" onclick="updateDeleteButton()">
                                    <div class="flex-1">
                                        <div class="flex items-start gap-3 mb-3">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                @if (($assignment->tipe === 'essay' || $assignment->tipe === 'praktik') && $question->kunci_jawaban)
                                                    <div
                                                        class="mb-3 p-3 bg-green-50 border-l-4 border-green-500 rounded-lg">
                                                        <p class="text-sm font-bold text-green-800 mb-1">Kunci Jawaban:
                                                        </p>
                                                        <p class="text-sm text-green-700 leading-relaxed">
                                                            {{ $question->kunci_jawaban }}</p>
                                                    </div>
                                                @endif
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        class="inline-flex items-center gap-1 px-3 py-1 text-sm font-bold text-purple-700 bg-purple-100 rounded-lg">
                                        ...
                                        @if ($assignment->tipe === 'pilihan_ganda')
                                            <div class="ml-11 space-y-2 mt-3">
                                                @foreach ($question->options as $opt)
                                                    <div
                                                        class="flex items-start gap-3 p-3 rounded-lg {{ $opt->is_correct ? 'bg-green-50 border-2 border-green-300' : 'bg-gray-50' }}">
                                                        <span
                                    ...
                                    <div class="flex flex-col gap-2">
                                        <button @click="edit=true"
                                            class="inline-flex items-center gap-1 px-4 py-2 text-sm font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-all">
                                        ...
                                        <form method="POST"
                                            action="{{ route('guru.questions.delete', $question->id_question) }}"
                                            onsubmit="return confirm('Yakin ingin menghapus soal ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="w-full inline-flex items-center gap-1 px-4 py-2 text-sm font-semibold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-all">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                            ...
                            <form x-show="edit" method="POST"
                                action="{{ route('guru.questions.update', $question->id_question) }}"
                                class="space-y-4">
                                @csrf
                                @method('PUT')
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 mb-2">Soal</label>
                                    <textarea name="soal" rows="2"
                                        class="w-full border-2 border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
                                        required>{{ $question->soal }}</textarea>
                                </div>
                                ...
                                @if ($assignment->tipe === 'essay' || $assignment->tipe === 'praktik')
                                    <div>
                                        ...
                                        <textarea name="kunci_jawaban" rows="2"
                                            class="w-full border-2 border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none">{{ $question->kunci_jawaban }}</textarea>
                                    </div>
                                @endif

                                <div>
                                    <label class="block text-sm font-bold text-gray-700 mb-2">Poin</label>
                                    <input type="number" name="poin" value="{{ $question->poin }}"
                                        class="w-full border-2 border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                </div>

                                @if ($assignment->tipe === 'pilihan_ganda')
                                    <div>
                                        <label class="block text-sm font-bold text-gray-700 mb-3">Pilihan
                                            Jawaban</label>
                                        <div class="space-y-2">
                                            @foreach ($question->options as $i => $opt)
                                                <div class="flex gap-3 items-center bg-gray-50 p-3 rounded-lg">
                                                    ...
                                                    <input type="text" name="pilihan[]"
                                                        value="{{ $opt->pilihan }}"
                                                        class="flex-1 border-2 border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                                    <span
                                                        class="text-sm font-semibold text-gray-400">{{ chr(65 + $i) }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                ...
                                <div class="flex gap-3 pt-4">
                                    <button type="submit"
                                        class="flex-1 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 text-white text-sm px-4 py-2.5 rounded-xl font-semibold hover:from-slate-800 hover:via-blue-800 hover:to-indigo-700 hover:scale-105 transition-all duration-300 shadow-lg shadow-indigo-500/50 hover:shadow-indigo-600/70">
                                        Simpan Perubahan
                                    </button>
                                    <button type="button" @click="edit=false"
                                        class="px-4 py-2.5 border-2 border-gray-300 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-all">
                                        Batal
                                    </button>
                                </div>
                            </form>
                ...
                @if ($assignment->questions->count() > 3)
                    <div class="text-center mt-6">
                        <button id="showAllBtn" onclick="toggleShowAll()"
                            class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-800 text-white rounded-xl hover:from-slate-900 hover:via-blue-900 hover:to-indigo-800 transition font-semibold shadow-md">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7" />
                            </svg>
        ...
        <div id="aiGenerateModal" class="fixed inset-0 z-50 hidden bg-black/40 backdrop-blur-sm animate-backdrop">
            <div class="flex items-end sm:items-center justify-center min-h-screen w-full">
                <div
                    ...
                    <button onclick="closeAIGenerateModal()"
                    ...
                    <div class="px-8 pt-8 text-center">
                        <div
                            class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 shadow-lg">
                        ...
                        <p class="text-gray-600 mb-6">Upload file PDF/DOC berisi soal dan kunci jawaban, AI akan
                            otomatis menginputkannya</p>
                    </div>

                    <form method="POST" action="{{ route('guru.questions.generate', $assignment->id_assignment) }}"
                        enctype="multipart/form-data" class="px-8 pb-8 space-y-6" id="aiGenerateForm">
                        @csrf
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-3">Upload File Soal</label>
                            <div
                                class="relative flex flex-col items-center justify-center w-full h-48 rounded-2xl border-3 border-dashed border-blue-300 bg-blue-50 cursor-pointer hover:bg-blue-100 transition">
                                <input id="aiFileInput" type="file" name="file" accept=".pdf,.doc,.docx,.txt"
                        ...
                        <div
                            class="bg-gradient-to-r from-blue-50 to-cyan-50 border-l-4 border-blue-500 p-4 rounded-lg">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="text-sm text-blue-800">
                                    ...
                                    <ul class="list-disc list-inside space-y-1 text-xs">
                                        ...
                                        <li><strong>Tandai jawaban benar: Jawaban: A atau Kunci: B atau Benar:
                                                C</strong></li>
                                        <li>Untuk essay: tambahkan kunci jawaban setelah soal</li>
                                        <li><strong>Format kunci essay: Kunci Jawaban: ... atau Jawaban: ...</strong>
                                        </li>
                        ...
                        <div class="flex flex-col-reverse sm:flex-row gap-3">
                            <button type="button" onclick="closeAIGenerateModal()"
                                class="flex-1 rounded-xl border-2 border-gray-300 px-6 py-3 text-gray-700 hover:bg-gray-50 transition font-semibold">
                                Batal
                            </button>
                            <button type="submit"
                                class="flex-1 rounded-xl bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-800 px-6 py-3 font-bold text-white hover:from-slate-800 hover:via-blue-800 hover:to-indigo-700 hover:scale-105 transition-all duration-300 shadow-lg shadow-indigo-500/50 hover:shadow-indigo-600/70 flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                function updateFileName(input) {
                    ...
                    if (input.files && input.files[0]) {
                        ...
                        fileNameDisplay.innerHTML =
                            '<svg class="w-3.5 h-3.5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>' +
                            input.files[0].name;
