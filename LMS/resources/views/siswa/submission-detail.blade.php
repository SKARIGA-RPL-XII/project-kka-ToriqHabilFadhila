...
<body class="bg-gray-50">
    ...
    <div class="w-full px-4 sm:px-6 md:px-16 py-12">
        <div
            class="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl p-8 mb-8 text-white animate-fade-in-up overflow-hidden relative">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <div class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl">
                    <span class="font-semibold">Dikumpulkan:
                ...
                @if ($submission->status === 'graded')
                    <div class="bg-green-500/30 backdrop-blur-sm px-4 py-2 rounded-xl border-2 border-green-300">
                        <span class="font-bold">Nilai:
                            {{ $submission->score }}/{{ $submission->assignment->max_score }}</span>
                    </div>
                @else
        ...
        <div class="bg-white rounded-3xl shadow-xl p-8">
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-2 border-blue-200">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-16 h-16 bg-blue-500 rounded-xl flex items-center justify-center flex-shrink-0">
                            ...
                            <a href="{{ Storage::url($submission->file_path) }}" target="_blank"
                                class="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition font-semibold shadow-md flex items-center gap-2">
                                    ...
                                    @foreach ($question->options as $optIndex => $option)
                                        @php
                                            $borderClass = match (true) {
                                                $option->id_option == $answer && $option->is_correct
                                        ...
                                        @endphp
                                        <div
                                            class="flex items-center gap-3 p-4 border-2 rounded-xl {{ $borderClass }}">
                                            @php
                                            ...
                                            @endphp
                                            <div
                                                class="w-6 h-6 rounded-full border-2 flex items-center justify-center {{ $radioClass }}">
                                            ...
                                            <span
                                                class="flex-1 {{ $option->id_option == $answer ? 'font-semibold' : '' }}">
            ...
            @if ($submission->status === 'graded' && $submission->graded_at)
                <div
                    class="mt-8 p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border-2 border-green-200">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Dinilai pada:
                                {{ $submission->graded_at->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="text-gray-700">Nilai Akhir:</p>
                        <p class="text-3xl font-bold text-green-600">
                            {{ $submission->score }}/{{ $submission->assignment->max_score }}</p>
                    </div>
                </div>
            @endif
        </div>
