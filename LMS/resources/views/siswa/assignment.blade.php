...
<body class="bg-gray-50">
    ...
    <div class="w-full px-4 sm:px-6 md:px-12 py-8">
        <div
            class="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl shadow-xl p-6 mb-6 text-white animate-fade-in-up overflow-hidden relative">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if ($existingSubmission)
                    <div
                        class="bg-green-500/30 backdrop-blur-sm px-4 py-2 rounded-xl border-2 border-green-300 flex items-center gap-1.5">
        ...
        @if ($existingSubmission)
            <div class="bg-white rounded-3xl shadow-xl p-8 text-center">
                ...
                <p class="text-gray-600 mb-6">Kamu sudah mengumpulkan tugas ini pada
                    {{ $existingSubmission->submitted_at->format('d M Y, H:i') }}</p>
                @if ($existingSubmission->status === 'graded')
                    <div class="inline-block bg-green-50 border-2 border-green-200 rounded-xl p-6 mb-6">
                        ...
                        <p class="text-4xl font-bold text-green-600">
                            {{ $existingSubmission->score }}/{{ $assignment->max_score }}</p>
                    </div>
                @else
                ...
                <div class="flex justify-center gap-4">
                    <a href="{{ route('siswa.submissions.show', $existingSubmission->id_submission) }}"
                        class="px-4 py-2.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white text-sm rounded-xl hover:from-indigo-700 hover:via-purple-700 hover:to-pink-700 transition font-semibold shadow-md">
                        Lihat Detail Jawaban
                    </a>
                    <a href="{{ route('dashboard') }}"
                        class="px-4 py-2.5 border-2 border-gray-300 text-gray-700 text-sm rounded-xl hover:bg-gray-50 transition font-semibold">
                            ...
                            <div class="mb-8 pb-8 {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                                ...
                                <div class="space-y-3">
                                    @foreach ($question->options as $optIndex => $option)
                                        <label
                                            class="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-500 hover:bg-indigo-50 cursor-pointer transition">
                                            <input type="radio" name="answers[{{ $question->id_question }}]"
                                                value="{{ $option->id_option }}" class="w-5 h-5 text-indigo-600"
                                                required>
                                            <span class="text-gray-800">{{ chr(65 + $optIndex) }}.
                                                {{ $option->pilihan }}</span>
                                        </label>
                                    @endforeach
                                </div>
                    ...
                    <form method="POST" action="{{ route('siswa.assignments.submit', $assignment->id_assignment) }}">
                        ...
                        @foreach ($assignment->questions as $index => $question)
                            <div class="mb-8">
                                ...
                                <textarea name="answers[{{ $question->id_question }}]" rows="5"
                                    class="w-full rounded-xl border-2 border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                    placeholder="Tulis jawabanmu di sini..." required></textarea>
                            </div>
                        @endforeach
                    ...
                    <form method="POST" action="{{ route('siswa.assignments.submit', $assignment->id_assignment) }}"
                        ...
                        <div class="max-w-2xl mx-auto">
                            <div class="text-center mb-8">
                                ...
                                <p class="text-gray-600">Upload file hasil pekerjaanmu (dokumen, presentasi, kode,
                                    video, dll)</p>
                            </div>

                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-900 mb-3">File Tugas</label>
                                <div id="dropzonePraktik"
                                    class="relative flex flex-col items-center justify-center w-full h-48 rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 cursor-pointer hover:bg-gray-100 transition">
                                    <input id="fileInputPraktik" type="file" name="file"
                                        accept=".pdf,.doc,.docx,.ppt,.pptx,.zip,.rar,.7z,.tar,.gz,.jpg,.jpeg,.png,.mp4,.avi,.mov"
                                ...
                                <p class="text-xs text-gray-500 mt-2">Format: PDF, DOC, DOCX, PPT, PPTX, ZIP, RAR, 7Z,
                                    JPG, PNG, MP4 (Max 50MB)</p>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-900 mb-3">Catatan Tambahan
                                    (Opsional)</label>
                                <textarea name="jawaban" rows="3"
                                    class="w-full rounded-xl border-2 border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                    placeholder="Tambahkan catatan atau penjelasan tentang tugas yang kamu kerjakan..."></textarea>
                            </div>
