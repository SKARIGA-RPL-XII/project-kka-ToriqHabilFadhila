...
<body class="bg-gray-50">
    ...
    <div class="w-full px-4 sm:px-6 md:px-16 py-12">
        <div
            class="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl p-8 mb-8 text-white overflow-hidden relative">
            ...
            <div class="relative z-10">
                <div class="flex items-start sm:items-center justify-between gap-3 mb-2">
                    ...
                    <button onclick="refreshRecommendations()"
                        class="flex-shrink-0 px-3 py-2 sm:px-4 sm:py-2 bg-white/20 hover:bg-white/30 rounded-xl text-white text-sm font-semibold transition-all flex items-center gap-2 backdrop-blur-sm border border-white/30 whitespace-nowrap">
                ...
                <p class="text-base sm:text-lg text-purple-100">AI akan menganalisis performa belajarmu dan memberikan
                    rekomendasi materi yang sesuai</p>
            </div>
        </div>
                        ...
                        if (decodedHTML.trim().startsWith('[') || decodedHTML.trim().startsWith('{')) {
                            try {
                                ...
                                recommendations.forEach(rec => {
                                    ...
                                    const youtubeLink =
                                        `https://www.youtube.com/results?search_query=${encodeURIComponent(title + ' tutorial')}`;
                                    const googleLink =
                                        `https://www.google.com/search?q=${encodeURIComponent(title)}`;

                                    html +=
                                        `<div class='mb-4 p-4 sm:p-5 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border-2 border-indigo-200 shadow-sm hover:shadow-md transition-shadow'>`;
                                    html += `<div class='flex flex-col sm:flex-row items-start gap-3 mb-3'>`;
                                    html +=
                                        `<div class='flex-shrink-0 w-8 h-8 bg-indigo-600 text-white rounded-full flex items-center justify-center font-bold text-sm'>${counter}</div>`;
                                    html += `<div class='flex-1'>`;
                                    html +=
                                        `<h3 class='font-bold text-gray-900 text-base sm:text-lg mb-2'>${title}</h3>`;
                                    html +=
                                        `<p class='text-xs sm:text-sm text-gray-700 leading-relaxed'>${description}</p>`;
                                    html += `</div></div>`;

                                    if (resources) {
                                        html +=
                                            `<div class='flex items-center gap-2 mb-3 text-xs text-gray-600 bg-white/50 px-3 py-2 rounded-lg'>`;
                                        html +=
                                        ...
                                        html +=
                                            `<span class='text-xs'><strong>Sumber Belajar:</strong> ${resources}</span></div>`;
                                    }

                                    html += `<div class='flex flex-col sm:flex-row flex-wrap gap-2'>`;
                                    html +=
                                        `<a href='${youtubeLink}' target='_blank' class='inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white text-xs font-semibold rounded-lg transition-all shadow-md hover:shadow-lg'>`;
                                    html +=
                                        `<svg class='w-4 h-4 flex-shrink-0' fill='currentColor' viewBox='0 0 24 24'><path d='M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z'/></svg>`;
                                    html += `<span>Video Tutorial</span></a>`;
                                    html +=
                                        `<a href='${googleLink}' target='_blank' class='inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white text-xs font-semibold rounded-lg transition-all shadow-md hover:shadow-lg'>`;
                                    html +=
                                        `<svg class='w-4 h-4 flex-shrink-0' fill='currentColor' viewBox='0 0 24 24'><path d='M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z'/></svg>`;
                                ...
                                recommendationsDiv.innerHTML =
                                    `<p class="text-red-600">Gagal memformat rekomendasi: ${e.message}</p>`;
                            }
                        } else {
                            // It's already HTML
